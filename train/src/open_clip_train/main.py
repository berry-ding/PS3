...
def main(args):
    ...
    if args.train_token_selection_only:
        model.lock_everything_but_token_selection()
    ...
    if args.train_data or (args.train_data_image and args.train_data_text_box) or args.dataset_type == "synthetic":
        ...
        if opt.startswith('timm/'):
            assert args.token_selection_lr is None, 'Haven\'t supported using timm optimizer with token selection learning rate.'
            ...
            is_token_selection = lambda n, p: any([x in n for x in model.module.token_selection_param_names()])

            named_parameters = list(model.named_parameters())
            if args.token_selection_lr is not None:
                token_selection_params = [p for n, p in named_parameters if is_token_selection(n, p) and p.requires_grad]
                gain_or_bias_params = [p for n, p in named_parameters if exclude(n, p) and p.requires_grad and not is_token_selection(n, p)]
                rest_params = [p for n, p in named_parameters if include(n, p) and p.requires_grad and not is_token_selection(n, p)]
                param_groups = [
                    {"params": gain_or_bias_params, "weight_decay": 0.},
                    {"params": token_selection_params, "lr": args.token_selection_lr, "weight_decay": 0.},

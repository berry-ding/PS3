...
from ps3 import PS3VisionConfig, PS3VisionEncoder
...
def _build_vision_tower(
    ...
    if isinstance(vision_cfg, dict):
        if vision_cfg.get("ps3", False):
            vision_cfg = PS3VisionConfig(**vision_cfg)
        else:
    ...
    if isinstance(vision_cfg, PS3VisionConfig):
        visual = PS3VisionEncoder(vision_cfg)
...
class CustomTextCLIP(nn.Module):
    ...
    def lock_everything_but_token_selection(self):
        for name, param in self.named_parameters():
            if "prompt_proj" not in name and "visual.selection_feature_proj" not in name and "visual.prior_prompt" not in name and "visual.highres_selection_feature_module" not in name and "visual.prompt_proj_for_highres" not in name:
                param.requires_grad = False
    
    def token_selection_param_names(self):
        return ["prompt_proj", "visual.selection_feature_proj", "visual.prior_prompt", "visual.highres_selection_feature_module", "visual.prompt_proj_for_highres"]

    @torch.jit.ignore
    def set_grad_checkpointing(self, enable=True):
        if self.ps3:
            self.visual.trunk.set_grad_checkpointing(True)
        else:
            self.visual.set_grad_checkpointing(enable)
        self.text.set_grad_checkpointing(enable)

    def encode_image(self, image, prompt=None, gt_selection_maps=None, is_global_text=None, normalize: bool = False, return_selection_map=False, pool_gt_token_only=False):
        if self.ps3:
            if return_selection_map:
                outputs = self.visual(image, prompt, gt_selection_maps, is_global_text, pool_gt_token_only=pool_gt_token_only)
                features, selection_maps, selection_probs, prior_selection_probs, posterior_selection_probs = outputs.pooled_output, outputs.selection_maps, outputs.selection_probs, outputs.bottomup_selection_probs, outputs.topdown_selection_probs
                return F.normalize(features, dim=-1) if normalize else features, selection_maps, selection_probs, prior_selection_probs, posterior_selection_probs
            features = self.visual(image, prompt, gt_selection_maps, is_global_text, pool_gt_token_only=pool_gt_token_only).pooled_output
        else:
            features = self.visual(image)
        return F.normalize(features, dim=-1) if normalize else features
    ...
    def forward_intermediates(
        ...
        """
        assert not self.ps3, "PS3 does not support forward_intermediates"
        output = {}
        if intermediates_only:
            # intermediates only disables final feature normalization, and include logits
            normalize = False
    ...
    def forward(
            ...
            image: Optional[torch.Tensor] = None,
            text: Optional[torch.Tensor] = None,
            gt_selection_maps: Optional[torch.Tensor] = None,
            is_global_text: Optional[torch.Tensor] = None,
            pool_gt_token_only: bool = False,
    ):
        text_features = self.encode_text(text, normalize=True) if text is not None else None

        if self.ps3:
            if text is None or text.shape[0] != image.shape[0]:
                prompt = None
            else:
                prompt = self.prompt_proj(text_features)
            image_features, selection_maps, selection_probs, prior_selection_probs, posterior_selection_probs = self.encode_image(image, prompt, gt_selection_maps, is_global_text, normalize=True, return_selection_map=True, 
                                                                                                                                  pool_gt_token_only=pool_gt_token_only) if image is not None else None
        else:
            image_features = self.encode_image(image, normalize=True) if image is not None else None
        ...
        if self.output_dict:
            ...
            if self.logit_bias is not None:
                out_dict['logit_bias'] = self.logit_bias
            if self.ps3:                
                out_dict['selection_maps'] = selection_maps
                out_dict['selection_probs'] = selection_probs
                out_dict['prior_selection_probs'] = prior_selection_probs
                out_dict['posterior_selection_probs'] = posterior_selection_probs
            return out_dict

...
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, SigLipLossPS3
...
def load_checkpoint(
    ...
    if 'logit_bias' not in state_dict and 'logit_scale' in state_dict and model.logit_bias is not None:
...
def create_model(
    ...
    # read preprocess config from model_config so it doesn't need huggingface config
    if "preprocess_cfg" in model_cfg:
        preprocess_cfg = merge_preprocess_dict(preprocess_cfg, model_cfg["preprocess_cfg"])
        model_cfg.pop("preprocess_cfg")
    ...
    if model_cfg["vision_cfg"].get('ps3', False):
        # for PS3, use max scale as the image size for preprocessing
        force_preprocess_cfg['size'] = (max(model.visual.ps3_scales), max(model.visual.ps3_scales))
    elif getattr(model.visual, 'image_size', None) is not None:
...
def create_loss(args):
    if args.distill:
        ...
        if "ps3" in args.model.lower():
            return SigLipLossPS3(
                rank=args.rank,
                world_size=args.world_size,
                selection_prob_loss_weight=args.selection_prob_loss_weight,
                prior_box_supervision_loss_weight=args.prior_box_supervision_loss_weight,
                posterior_box_supervision_loss_weight=args.posterior_box_supervision_loss_weight,
            )
        else:
            return SigLipLoss(
                rank=args.rank,
